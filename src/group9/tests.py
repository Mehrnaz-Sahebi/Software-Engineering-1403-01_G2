...
class UserSignupTestCase(TestCase):
    ...
    def test_signup_password_mismatch(self):
        ...
        response = self.client.post(reverse('group9:signup'), data)
        self.assertContains(response, "Your password and confirm password are not the same!")
...
class UserLoginTestCase(TestCase):
    ...
    def test_login_valid_user(self):
        ...
        response = self.client.post(reverse('group9:login'), {'username': 'testuser', 'pass': 'password123'})
        self.assertEqual(response.status_code, 302)  # Redirects to the optimization page
...
class TextOptimizationTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='userToCheckOptimization', password='password123')
...
class UserHistoryTestCase(TestCase):
    def setUp(self):
        ...
        self.mock_db_connection.cursor.return_value.fetchall.return_value = [
            ('input_text_example', 'remove_special_chars', 'Special characters removed', 'correct form example', '2025-01-13')
        ]
